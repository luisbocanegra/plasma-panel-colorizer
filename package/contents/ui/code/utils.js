...
function getSystemTrayState(applet, plasmaVersion) {
    ...
    if (plasmaVersion.isGreaterThan("6.3.5")) {
        ...
        systemTrayState =
            "internalSystray" in applet && applet.internalSystray.systemTrayState;
    }
...
function getWidgetProperties(
    ...
    if (item.applet?.plasmoid?.pluginName) {
        ...
        needsAttention =
            pcTypes.NeedsAttentionStatus === applet.plasmoid.status ?? null;
            ...
            if (model.itemType === "StatusNotifier") {
                ...
                if (panelColorizer && iconSource) {
                    if (typeof panelColorizer.getIconHash !== "function") {
                        console.error(
                            "getIconHash is not a method of",
                            panelColorizer,
                            "please update/rebuild the plugin using latest source",
                        );
                    } else {
                        try {
...
function getElementStateConfig(config, state) {
    ...
    if (state.needsAttention && config.needsAttention?.enabled) {
        stateConfig = Utils.getEffectiveSettings(
...
function getItemCfg(
    ...
    let globalOverrideConfig = getCustomCfg(
        widgetName,
        widgetId,
        globalOverrides,
    );
    let globalOverride = getElementStateConfig(globalOverrideConfig, state);
    let presetOverride = getElementStateConfig(
        getCustomCfg(widgetName, widgetId, config.configurationOverrides),
        state,
    );
...
function mergeConfigs(sourceConfig, newConfig) {
    ...
    for (var key in sourceConfig) {
        if (Array.isArray(sourceConfig[key])) {
            ...
            typeof sourceConfig[key] === "object" &&
    ...
    if (Object.keys(configurationOverrides)) {
        Object.keys(configurationOverrides).forEach((key) => {
            if (!("normal" in configurationOverrides[key])) {
                ...
                configurationOverrides[key].disabledFallback =
                    current.disabledFallback ?? true;
            }
        });
    }
}
...
function fixGlobalSettingsV3(globalSettings) {
    ...
    if (
        globalSettings?.panel?.normal?.floatingDialogs !== undefined &&
        "floatingDialogs" in globalSettings.panel.normal.floatingDialogs
    ) {
    ...
    if (
        "nativePanelBackground" in globalSettings &&
        !("nativePanel" in globalSettings)
    ) {
        nativePanelBackground =
            globalSettings.nativePanelBackground?.enabled ?? true;
    ...
    fixConfigurationOverridesV3(
        globalSettings.configurationOverrides?.overrides ?? null,
    );
}
...
function showWidgets(panelLayout, backgroundComponent, Plasmoid) {
    ...
    for (var i in panelLayout.children) {
        ...
        if (!child.applet?.plasmoid?.pluginName) continue;
        const isTray =
            child.applet.plasmoid.pluginName === "org.kde.plasma.systemtray";
...
function getColor(
    ...
    newColor = Qt.hsla(
        newColor.hslHue,
        newColor.hslSaturation,
        newColor.hslLightness,
        colorCfg.alpha,
    );
    return newColor;
}
...
function applyFgColor(
    ...
    for (var i = 0; i < element.visibleChildren.length; i++) {
        ...
        if (
            ...
            if (
                forceEffect &&
                [Canvas, Kirigami.Icon].some(function (type) {
                    return child instanceof type;
                })
            ) {
...
function getTrayIconFromRules(rules, trayItemProperties) {
    const filtered = rules.filter((rule) => {
        return (
            rule.enabled && rule.hash !== "" && rule.match !== "" && rule.icon !== ""
        );
    });
    return (
        filtered.find((rule) => {
            ...
            return (
                re.test(trayItemProperties.title) || re.test(trayItemProperties.name)
            );
        })?.icon ?? ""
    );
}
